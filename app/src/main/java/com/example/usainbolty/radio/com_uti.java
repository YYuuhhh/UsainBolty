...
//        <device name="SND_DEVICE_IN_CAMCORDER_MIC" acdb_id="0" />
	...
	public static void acdbfix_install(Context context) {
		...
		try {
			...
			fixed_once = false;
			BufferedReader br = new BufferedReader(new FileReader(platform_file), 8192);//512);  // Should never need more than 512 bytes per line
			while (true) {
				line = br.readLine();
				if (line == null) {
					bw.close();                                                  // Close output file
	...
	private static void acdbfix_copy() {
		...
		String cmd = "";
		cmd += ("mount -o remount,rw /system ; ");
		if (com_uti.isFileExists(platform_file) && !com_uti.isFileExists(platform_orig)) {   // !! Only copy to orig if orig does not already exist !
...
*/
	...
	public static boolean isFileExists(String filename) {
		return (isFileExists(filename, true));
	}
	...
	public static boolean isHTC() {
		return (isFileExists("/sys/class/htc_accessory/fm/flag") || sys_prop_manuf.startsWith("HTC"));// || stock_htc_get ());
	}

	// Stock Manuf:
	public static boolean stock_mot_get() {
		return (isFileExists("/system/framework/com.motorola.blur.library.app.service.jar") || isFileExists("/system/framework/com.motorola.frameworks.core.addon.jar"));
	}

	public static boolean stock_lg2_get() {
		return (com_uti.isFileExists("/system/framework/com.lge.systemservice.core.jar") || com_uti.isFileExists("/system/framework/com.lge.core.jar") || com_uti.isFileExists("/system/framework/com.lge.frameworks.jar"));
	}

	// Device:
	public static boolean isOM8() {
	...
	// For One M7 and One Mini, which has same FM
	public static boolean isOM7() {
		return (sys_prop_device.startsWith("M7") || (sys_prop_device.startsWith("M4") && isHTC()) || sys_prop_device.startsWith("HTC_M4"));
	}

	private static boolean isGalaxyNote2() {
		return sys_prop_device.startsWith("T03G") ||                          // Galaxy Note2 3G
	...
	public static boolean isMotorolaG() {
...
*/
	...
	public static String chass_plug_aud_get(Context context) {
		...
		else if (isGalaxyNote2())
			com_uti.chass_plug_aud = "GS3";

			//else if (com_uti.isSony () && isFileExists ("/system/lib/libbt-fmrds.so"))      // DISABLED: Was causing many QCV Sony's to be mis-identified  // ? Z2/Z3 need to be more specific ?
...
//C65, C66, C69
		...
		else if (com_uti.isOM8())                                        // HTC One M8 2014
			com_uti.chass_plug_aud = "QCV";
		...
		if (com_uti.chass_plug_aud.equals("UNK")) {
			if (com_uti.isFileExists("/dev/radio0") && com_uti.isFileExists("/sys/devices/platform/APPS_FM.6")) {   // Qualcomm is always V4L and has this FM /sys directory
				com_uti.chass_plug_aud = "QCV";
			} else if (com_uti.isFileExists("/dev/radio0") || com_uti.isFileExists("/dev/fmradio")) {                 // Samsung GalaxyS class devices always have one of these driver names for Samsung Silicon Labs driver
				if (com_uti.isFileExists("/sys/kernel/debug/asoc/smdkc110/wm8994-samsung-codec.4-001a/codec_reg"))
					com_uti.chass_plug_aud = "GS1";
				else if (com_uti.isFileExists("/sys/kernel/debug/asoc/U1-YMU823") || com_uti.isFileExists("/sys/devices/platform/soc-audio/MC1N2 AIF1") || com_uti.isFileExists("/sys/kernel/debug/asoc/U1-YMU823/mc1n2.6-003a"))
					com_uti.chass_plug_aud = "GS2";
				else if (com_uti.isFileExists("/sys/kernel/debug/asoc/T0_WM1811/wm8994-codec/codec_reg") || com_uti.isFileExists("/sys/kernel/debug/asoc/Midas_WM1811/wm8994-codec/codec_reg") || com_uti.isFileExists("/sys/devices/platform/soc-audio/WM8994 AIF1/codec_reg"))
					com_uti.chass_plug_aud = "GS3";
			} else {                                                                          // Only remaining alternative is Broadcom
				if (com_uti.isFileExists("/dev/ttyHS99") && com_uti.isFileExists("/sys/class/g2_rgb_led"))
					com_uti.chass_plug_aud = "LG2";
				else if (com_uti.isFileExists("/dev/ttyHS0") && (com_uti.isFileExists("/sys/devices/platform/m7_rfkill") || com_uti.isFileExists("/sys/devices/platform/mipi_m7.0") || com_uti.isFileExists("/sys/module/board_m7_audio")))
					com_uti.chass_plug_aud = "OM7";
			}
